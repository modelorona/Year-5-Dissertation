...
import com.anguel.dissertation.ml.smile.feature.TreeSHAP;
...
public class RandomForest implements SoftClassifier<Tuple>, DataFrameClassifier, TreeSHAP {
    ...
    @Setter // sent to server
    private ModelStats modelStats;

    // attempts to make any of the double values "safe"
    public ModelStats getModelStats() {
        final double safeDouble = -9999.00;
        if (Double.isNaN(this.modelStats.getAccuracy())) this.modelStats.setAccuracy(safeDouble);
        if (Double.isNaN(this.modelStats.getSensitivity())) this.modelStats.setSensitivity(safeDouble);
        if (Double.isNaN(this.modelStats.getSpecificity())) this.modelStats.setSpecificity(safeDouble);
        if (Double.isNaN(this.modelStats.getPrecision())) this.modelStats.setPrecision(safeDouble);
        if (Double.isNaN(this.modelStats.getFalseDiscoveryRate())) this.modelStats.setFalseDiscoveryRate(safeDouble);
        if (Double.isNaN(this.modelStats.getFalsePositiveRate())) this.modelStats.setFalsePositiveRate(safeDouble);
        if (Double.isNaN(this.modelStats.getF1score())) this.modelStats.setF1score(safeDouble);
    ...
    public RandomForest(Formula formula, int k, Model[] models, ClassificationMetrics metrics, double[] importance, IntSet labels, ModelStats stats) {
        this(formula, k, models, metrics, importance, labels);
    ...
    public static RandomForest fit(Formula formula, DataFrame data, int ntrees, int mtry,
        ...
        Model[] models = Arrays.stream(seedArray).mapToObj(seed -> {
...
//                logger.error("Random forest has a tree trained without OOB samples.");
    ...
    public RandomForest merge(RandomForest other) {
        ...
        ModelStats mergedStats = ModelStats.builder()
                .testSize(this.modelStats.getTestSize() + other.getModelStats().getTestSize())
                .trainSize(this.modelStats.getTrainSize() + other.getModelStats().getTrainSize())
                .specificity((this.modelStats.getSpecificity() * other.getModelStats().getSpecificity()) / 2)
                .sensitivity((this.modelStats.getSensitivity() * other.getModelStats().getSensitivity()) / 2)
                .precision((this.modelStats.getPrecision() * other.getModelStats().getPrecision()) / 2)
                .falsePositiveRate((this.modelStats.getFalsePositiveRate() * other.getModelStats().getFalsePositiveRate()) / 2)
                .falseDiscoveryRate((this.modelStats.getFalseDiscoveryRate() * other.getModelStats().getFalseDiscoveryRate()) / 2)
                .f1score((this.modelStats.getF1score() * other.getModelStats().getF1score()) / 2)
                .confusionMatrix(new ConfusionMatrix(averageConfusionMatrix(this.modelStats.getConfusionMatrix().matrix, other.getModelStats().getConfusionMatrix().matrix)))
                .accuracy((this.modelStats.getAccuracy() * other.getModelStats().getAccuracy()) / 2)
                .build();

        return new RandomForest(formula, k, forest, mergedMetrics, mergedImportance, labels, mergedStats);
    }

    // assume the confusion matrix is the same size since the class can be only values
    private int[][] averageConfusionMatrix(int[][] m1, int[][] m2) {
